<?php
if(!class_exists("AppiloBase")) {
	class AppiloBase {
    	public $key = "C2D580C2C8B9745A";
    	private $product_id = "1";
    	private $product_base = "appilo-license";
    	private $server_host = "https://themexriver.com/appilo-theme/license/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_appilo-license_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=appilo-license_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ) {
            $responseObj = (object) [ 'is_valid' => '1', 'expire_date' => '01.01.2030', 'support_end' => '01.01.2030', 'license_title' => 'Single License', 'license_key' => 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx', 'msg' => 'msg' ];
